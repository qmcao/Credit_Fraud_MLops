...
class DataTransformation:
    ...
    def find_column_to_drop(self, train_data: pd.DataFrame) -> list:
        """
        Identifies columns in a dataframe with more than 70% missing values.

        The decision is based on the training data to prevent data leakage.
    ...
    def clean_data(self, train_data: pd.DataFrame, test_data: pd.DataFrame, columns_to_drop: list[str]) -> pd.DataFrame:
        """
        This function handle missing data.
        """
        
        columns_to_drop = self.find_column_to_drop(train_data)
